...
<?php
 if(isset($_POST['type']) && $getUser['level'] == 'ctv')
 {
     ...
     {
         $is = $_POST['id'];
         $check=$NNL->get_row("SELECT * from `pk` where `id`='$is'");
         if(!$check)
         {
            admin_msg_warning("Đơn hàng không hợp lệ", "", 2000);
         }
         if($check['user']==$getUser['username'])
         {
            admin_msg_warning("Không thể nhận kèo chính bạn", "", 2000);
         }
            ...
            <div class="col-md-12">
                <div class="card card-outline card-primary">
                    <div class="card-header">
                        <h3 class="card-title">DANH SÁCH ĐƠN SOLO</h3>
                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($NNL->get_list("SELECT * FROM `pk` where `status`='0' AND `nguoinhan`='' ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td><?=$row['phong'];?></td>
                                        ...
                                        <td><?=$row['type'];?></td>
                                        <td><?=$row['tuong'];?></td>
                                        <td><?php if($row['status'] ==0) { echo '<span class="badge badge-warning">Đang đợi xác nhận</span>';} else if($row['status'] ==1) { echo '<span class="badge badge-success">Đã chấp nhận kèo</span>';};?></span></td>
                                        <td>
                                            <button class="btn btn-primary NhanHang"
                                            onclick="nhan(<?php echo $row['id'];?>)">
                                                <span>Nhận Kèo</span></button>
                                        </td>
                                    </tr>
                                    <?php }?>
                                </tbody>
