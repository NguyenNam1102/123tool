                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($NNL->get_list("SELECT * FROM `orders_caythue` where `nguoinhan`='' ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
                                        <td><?=$row['dichvu'];?></td>
                                        ...
                                        <td>
                                            ...
                                            onclick="nhan(<?php echo $row['id'];?>)">
                                                <span>Nhận Đơn Hàng</span></button>
                                        </td>
                                    </tr>
                                    <?php }?>
                                </tbody>
...
<script type="text/javascript">
        const nhan = (type) => {
            Swal.fire({
        title: 'Xóa nhận',
        text: "Bạn có đồng ý nhận đơn này không?",
        icon: 'info',
        ...
        if (result.isConfirmed) {
            $('.NhanHang').html('ĐANG XỬ LÝ').prop('disabled',
                true);
            $.ajax({
                url: "<?=BASE_URL("public/ctv/DonHangCayThue.php");?>",
                method: "POST",
                data: {
                    type: 'xacnhan',
                    id: type
