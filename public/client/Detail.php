...
if(isset($_GET['id']))
{
    $row = $NNL->get_row(" SELECT * FROM `accounts` WHERE `id` = '".check_string($_GET['id'])."'  ");
    if(!$row)
    {
    ...
    <div class="sticky col-span-12 grid grid-cols-10 gap-2 select-none py-2 px-2 color-grant text-xl font-bold rounded"
        ...
        <div class="col-span-10 md:col-span-5">
            <div class="flex items-center flex-wrap text-yellow-500 pt-3">
                <div class="relative">
                    <?=format_cash($row['money']);?> đ/1 tháng
        ...
        <div class="v-account-detail p-2 md:px-0 mt-4">
            <div class="v-account-detail-1" id="taikhoan">
                <div class="mb-10">
                    ...
                    <center><img src="<?=$b;?>" data-sizes="auto" class="border border-gray-400 mb-2 w-500 lazyLoad lazy" /></center>
                    <?php }?>
                </div>
            </div>
        </div>
