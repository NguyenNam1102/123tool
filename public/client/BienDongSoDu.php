<?php
    ...
    $title = 'BIẾN ĐỘNG SỐ DƯ | '.$NNL->site('tenweb');
                    ...
                    <div class="py-2 overflow-x-auto scrolling-touch max-w-400">
                        <table id="datatable" class="table-auto w-full scrolling-touch min-w-850">
                            ...
                            <tbody class="text-sm font-semibold">
                                <?php $i = 0; foreach($NNL->get_list(" SELECT * FROM `dongtien` WHERE `username` = '".$getUser['username']."' ORDER BY id DESC ") as $row){ ?>
                                <tr style="background-color: transparent;">
                                    <td class="text-sm text-white text-left px-1 py-1"><?=$i++;?></td>
                                    <td class="text-sm text-white text-left px-1 py-1"><?=format_cash($row['sotientruoc']);?> VNĐ</td>
                                    <td class="text-sm text-white text-left px-1 py-1"><?=format_cash($row['sotienthaydoi']);?> VNĐ</td>
                                    <td class="text-sm text-white text-left px-1 py-1"><?=format_cash($row['sotiensau']);?> VNĐ</td>
                                    <td class="text-sm text-white text-left px-1 py-1"><span class="badge badge-dark"><?=$row['thoigian'];?></span></td>
                                    <td class="text-sm text-white text-left px-1 py-1"><?=$row['noidung'];?></td>
                                </tr>
                                <?php }?>
                            </tbody>
                        </table>
